...
def Calc_sum(a1, a2, a3):
    ...
    for t in range(1, 41):
        ...
        for i in range(1, 316): #8*70 min(200.0 / 560.0 * float(car_dis[i]), 200)
            ...
            cost[i] = a1 * min(float(car_val[i]), 80.0) + a2 * min(80.0 * (float(car_dis[i]) / 4800.0), 80) + a3 * 150 * float(T_com[i])
        x1 = max(cost)
        x2 = max(v)
...
for i in range(4):
    if i == 0:
        ...
        a2 = 0.2
        a3 = 0.2
        rs = Calc_sum(a1, a2, a3)
    elif i == 1:
        a1 = 0.7
        a2 = 0.15
        a3 = 0.15
        rs1 = Calc_sum(a1, a2, a3)
    elif i == 2:
        a1 = 0.8
        a2 = 0.1
        a3 = 0.1
        rs2 = Calc_sum(a1, a2, a3)
    else:
        a1 = 0.9
        a2 = 0.05
        a3 = 0.05
        rs3 = Calc_sum(a1, a2, a3)
    
t = [i for i in range(1, 41)]
plt.title('Cost under different parameters', fontsize = 'x-large')
plt.xlabel('Time')
plt.ylabel('Cost(k)')
plt.plot(t, rs, 'r^-', linewidth = 1, markersize = 4, label = '${ρ_{1}}$= 0.6,${ρ_{2}}$=0.2,${ρ_{3}}$=0.2')
plt.plot(t, rs1, 'k*-', linewidth = 1, markersize = 4, label = '${ρ_{1}}$= 0.7,${ρ_{2}}$=0.15,${ρ_{3}}$=0.15')
plt.plot(t, rs2, 'b+-', linewidth = 1, markersize = 4, label = '${ρ_{1}}$= 0.8,${ρ_{2}}$=0.1,${ρ_{3}}$=0.1')
plt.plot(t, rs3, 'gs-', linewidth = 1, markersize = 4, label = '${ρ_{1}}$= 0.9,${ρ_{2}}$=0.05,${ρ_{3}}$=0.05')
...
plt.savefig('cost_diff_para.png')
plt.show()