...
#a = 1.15 q = 0.9
#ways, data2, id, w1=0.4,w2=0.3, w3=0.3
def opt2(ways, data2, id, w1=0.7,w2=0.3):
    a = 1.05
    q = 0.8
    W = float(id / 10.0)
    w3 = W
    print(w3)
    #print(str(id) + ':' + str(a) + ',' + str(q))
    num = len(data2)
...
def fullData2(centralData_X, centralData_Y,id,sensor_vis):
    ...
    for i in range(len(centralData_X)):
        ...
        Y = centralData_Y[i]
        for j in range(1000):
            if sensor_vis[sensor_id[j]]==0 and Distance.distance(sensor_X[sensor_id[j]],sensor_Y[sensor_id[j]],X,Y)<1500: # 核心传感器的半径
                if len(s_x) < 11:  # 初始选择的点数
...
'''
c = []
a = [1.0114337810903333, 1.0001093574233781, 1.0097801251278207, 1.0077543525817376, 1.0034314613210922, 1.0073085344378183, 1.0025460842707306, 1.0061700210930662, 1.0026175146091516]
b = [1.3333333333333333, 1.1666666666666667, 1.3333333333333333, 1.3333333333333333, 1.3333333333333333, 1.3333333333333333, 1.25, 1.25, 1.25]
