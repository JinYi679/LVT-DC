...
def draw(center_x,center_y,base_x,base_y,basesensor_x,basesensor_y,noreachsensor_x,noreachsensor_y,line_x,line_y,between_x,between_y,xticks,yticks):
    ...
    plt.title('Path optimization(Cycle=1)',fontsize = 'x-large')
    #plt.xticks(xticks)
    #plt.yticks(yticks)
    plt.xticks(np.arange(12.2, 13.0, 0.15))
    plt.yticks(np.arange(41.7, 42.5, 0.15))

    plt.scatter(base_x, base_y, s=0.01, c='#B3B3B5', alpha=0.5, label = 'Vehicle trajectory')
    plt.scatter(basesensor_x,basesensor_y,s=15,c='#7FFFAA',alpha=1, label = 'Covered sensor')
    plt.scatter(noreachsensor_x,noreachsensor_y,s=15,c='#CD5C5C',alpha=1, label = 'Uncovered sensor')
    
    
    for i in range(len(line_x)):
        if i == 0:
            plt.plot(line_x[i],line_y[i],c='b', label = 'Pathway')
        else:
            plt.plot(line_x[i],line_y[i],c='b')
    plt.scatter(between_x,between_y,s=18,c='b',alpha=1, label = 'Sensor of Pathway')
    ...
    for i in range(len(center_x)):
        ...
        if i == 0:
            plt.scatter(center_x[i],center_y[i], s=30 ,c='k', marker='*',alpha=1, label = 'Advanced data center')
            plt.plot(x, y, color='k', label = 'Flight range')
        else:
            plt.scatter(center_x[i],center_y[i], s=30 ,c='k', marker='*',alpha=1)
            plt.plot(x, y, color='k')
        plt.plot(x, y1, color='k')
    
    pic = plt.legend(loc = 'upper right',framealpha = 0.5, fontsize = 'medium')
    #调整图例散点的大小
    pic.legendHandles[0]._sizes = [30]
    pic.legendHandles[1]._sizes = [30]
    pic.legendHandles[2]._sizes = [30]
    pic.legendHandles[3]._sizes = [30]
    pic.legendHandles[4]._sizes = [30]
    pic.legendHandles[5]._sizes = [30]
    pic.legendHandles[6]._sizes = [30]
    #plt.savefig('Path optimization.png')
    plt.show()
'''
def draw(center_x,center_y,base_x,base_y,basesensor_x,basesensor_y,noreachsensor_x,noreachsensor_y,line_x,line_y,between_x,between_y,xticks,yticks):
    #plt.figure(figsize=(2.3, 1.69)) #宽/高
    #plt.title('Path optimization(Cycle=1)',fontsize = 'x-large')
    r=0.03
    ...
    #7FFFAA
    plt.scatter(base_x, base_y, s=0.01, c='#B3B3B5', alpha=0.5, label = 'MVs trajectory')
    plt.scatter(basesensor_x,basesensor_y,s=15,c='r',alpha=1, label = 'Sensor')
    plt.scatter(noreachsensor_x,noreachsensor_y,s=15,c='r',alpha=1)
    ...
    pic.legendHandles[2]._sizes = [30]
    pic.legendHandles[3]._sizes = [30]
    plt.savefig('p11.png', bbox_inches='tight', dpi = 600, pad_inches = 0.0)
    plt.show()
...
def readFindData1():
    ...
    with open('F:/Find Data/1.txt', 'r') as f:
        while f.readable():
            ...
            x = s.split(';')
            x[1] = x[1].strip()
            sensor_id.append(int(x[0])) #优化了读取文件的问题
...
def fullData(centralData_X, centralData_Y):
    ...
    for i in range(len(centralData_X)):
        ...
        Y = centralData_Y[i]
        for j in range(999):
            if Distance.distance(sensor_X[sensor_id[j]],sensor_Y[sensor_id[j]],X,Y)<1900: # 核心传感器的半径
...
if __name__ == '__main__':
    ...
    #,12.5165,12.4718 ,41.8803,41.8216
